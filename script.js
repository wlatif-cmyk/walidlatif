...
// music symbols fly around on spotify logo hover
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createMusicSymbol() {
        ...
        symbol.textContent = musicSymbols[Math.floor(Math.random() * musicSymbols.length)];
        
        // get spotify logo position
        const rect = spotifyLogo.getBoundingClientRect();
        ...
        symbol.style.transform = 'translate(-50%, -50%) scale(0.8) rotate(0deg)';
        ...
        symbol.style.transform = `translate(calc(-50% + ${flyX}px), calc(-50% + ${flyY}px)) scale(1.2) rotate(${rotate}deg)`;
    ...
    spotifyLogo.addEventListener('mouseenter', function() {
        isHovering = true;
        // create initial burst of symbols
        createMusicSymbol();
        setTimeout(() => createMusicSymbol(), 100);
        setTimeout(() => createMusicSymbol(), 200);
        setTimeout(() => createMusicSymbol(), 300);
        setTimeout(() => createMusicSymbol(), 400);
        
        // continue creating symbols while hovering (slower after burst)
    ...
    spotifyLogo.addEventListener('mouseleave', function() {
        isHovering = false;
        if (hoverInterval) {
...
// briefcases and resumes fly around on linkedin logo hover
document.addEventListener('DOMContentLoaded', function() {
    const linkedinLogo = document.querySelector('.linkedin-logo');
    if (!linkedinLogo) return;
    
    // business symbols to use
    const businessSymbols = ['💼', '📄', '📋', '📊', '📈', '💼', '📄', '📝'];
    
    let hoverInterval = null;
    let isHovering = false;
    
    function createBusinessSymbol() {
        const symbol = document.createElement('div');
        symbol.className = 'business-symbol';
        symbol.textContent = businessSymbols[Math.floor(Math.random() * businessSymbols.length)];
        
        // get linkedin logo position
    ...
    linkedinLogo.addEventListener('mouseenter', function() {
        isHovering = true;
        // create initial burst of symbols
        createBusinessSymbol();
        setTimeout(() => createBusinessSymbol(), 100);
        setTimeout(() => createBusinessSymbol(), 200);
        setTimeout(() => createBusinessSymbol(), 300);
        setTimeout(() => createBusinessSymbol(), 400);
